...
level_order <- c('Less than 5 hours.', '5-6 hours.', '6-7 hours.', '7-8 hours.', '9-10 hours.', 'More than 10 hours.')
#ggplot(rest_habits, aes(x = factor(sleepDuration, level = level_order), y = degree)) + geom_col()

rest_habits %>% 
  filter(!is.na(sleepDuration)) %>% 
  ggplot() +
  geom_mosaic(aes(x=product(factor(sleepDuration, level=level_order), degree), fill=sleepDuration)) +
...
```{r circle bar chart (grad degree vs. gpa)}
circle_grad %>% 
  ggplot(aes(x=degree, y=avg_gpa)) +
  geom_hline(
    aes(yintercept = y), 
    data.frame(y = c(0:4)),
    color = "lightgrey"
  ...
  geom_col(aes(
      x = reorder(str_wrap(degree, 5), avg_gpa),
      y = avg_gpa,
      fill = num_resp
    ),
    position = "dodge2",
