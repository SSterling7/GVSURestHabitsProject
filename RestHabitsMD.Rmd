...
```{r group majors}
#should I instead add categories to new column within rest habits?
# 
...
rest_habits <- rest_habits %>%
  mutate(category = case_when(
     grad %in% c("Communications", "Business Administration", "Criminal Justice") ~ "busLeg", #Business & Legal
     grad == "" ~ "cultHum", #Cultures and Humanities
     grad == "" ~ "science", #Biological, Environmental, and Physical Sciences
     grad %in% c("Education", "Higher Education", "School Psychology") ~ "education", #Education
     grad == "" ~ "art", #Fine Arts and Design
     grad %in% c("Physician Assistant Studies", "Medical Dosimetry", "Public Health", "Physical Therapy", "Nursing") ~ "health", #Health and Medicine
     grad %in% c("Biostatistics", "Data Science and Analytics") ~ "mathEng", #Math and Engineering
     grad %in% c("Speech-Language Pathology", "Social Work") ~ "socBehave")#Social and Behavioral Sciences)
  )
```
...
rest_habits <- rest_habits %>%
  mutate(category = case_when(
    undergrad %in% c("Marketing", "Accounting", "Finance", "International Business", "Advertising and Public Relations", "General Business",
                     "Management", "Hospitality and Tourism Management", "Human Resource Management", "Communication Studies",
                     "Criminal Justice", "Legal Studies") ~ "busLeg", #Business & Legal
    undergrad %in% c("French", "Spanish", "History", "English", "Writing") ~ "cultHum", #Cultures and Humanities
    undergrad == "Biology",  ~ "science", #Biological, Environmental, and Physical Sciences
    undergrad %in% c("Education", "Higher Education", "School Psychology") ~ "education", #Education
    undergrad == "" ~ "art", #Fine Arts and Design
    undergrad %in% c("Physician Assistant Studies", "Medical Dosimetry", "Public Health", "Physical Therapy", "Nursing") ~ "health", #Health and Medicine
    undergrad %in% c("Biostatistics", "Data Science and Analytics") ~ "mathEng", #Math and Engineering
    undergrad %in% c("Speech-Language Pathology", "Social Work") ~ "socBehave")#Social and Behavioral Sciences)
  )
```
...
rest_habits %>% 
  ...
  ggplot(aes(x=gpa, fill=leisureTime)) + 
  geom_histogram()
```
